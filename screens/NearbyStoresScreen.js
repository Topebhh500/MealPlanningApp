...
import { View, StyleSheet, Dimensions, Text, Linking, FlatList, TouchableOpacity } from 'react-native';
...
const NearbyStoresScreen = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      setAddress(`${geoAddress.street}, ${geoAddress.city}, ${geoAddress.region}, ${geoAddress.country}`);

      // Fetch nearby restaurants using Google Places API
      const response = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${location.coords.latitude},${location.coords.longitude}&radius=1500&type=supermarket&key=${GOOGLE_PLACES_API_KEY}`);
      ...
      const storesWithDistances = data.results.map(store => {
        ...
        return { ...store, distance }; // Add distance property to store
      });
  ...
  const renderStoreItem = ({ item }) => (
    <TouchableOpacity style={styles.storeItem} onPress={() => openMaps(item.geometry.location.lat, item.geometry.location.lng)}>
      <Text style={styles.storeName}>{item.name} - ({item.distance} km)</Text>
